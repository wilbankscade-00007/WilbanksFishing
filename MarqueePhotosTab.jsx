...
export default function MarqueePhotosTab({ entity }) {
  ...
  const loadPhotos = async () => {
    setLoading(true);
    try { setPhotos(await Entity.list()); } catch (e) { console.error(e); }
    finally { setLoading(false); }
  };

  const handleSave = async () => {
    try {
      if (editing.id) await Entity.update(editing.id, editing);
      else await Entity.create(editing);
  ...
  const handleDelete = async (id) => {
    ...
    try { await Entity.delete(id); loadPhotos(); }
    catch (e) { alert('Failed to delete.'); }
  };

  const heading = entity === 'CatchPhoto' ? 'Photo Marquee 1' : 'Photo Marquee 2';

  if (editing) {
    return (
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h3 className="font-heading text-xl text-[#E2E8F0] uppercase">{editing.id ? 'Edit' : 'Add'} {heading} Photo</h3>
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h3 className="font-heading text-xl text-[#E2E8F0] uppercase">{heading} ({photos.length})</h3>
      ...
      {loading ? (
        ...
        <p className="text-center text-[#E2E8F0]/40 text-sm py-8">No photos yet. Add photos to show in this marquee.</p>
      ) : (
